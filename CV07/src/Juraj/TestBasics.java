    ...
    public void testabecedaStream() {
        {
            ...
            List<String> res1 = myabecedaStream(ls).flatMap(st -> st).collect(Collectors.toList());
            List<String> res2 = Basics.abecedaStream(ls).flatMap(st -> st).collect(Collectors.toList());

            assertArrayEquals("abecedaStream test 1",
                    res1.toArray(new String[0]),
                    res2.toArray(new String[0]));
        }
        {
//            List<String> ls = Arrays.asList(spoluziaci);
            List<String> ls = Arrays.asList(spoluziaciCelejSkoly);
            ...
            assertArrayEquals("abecedaStream test 2",
                    res1.toArray(new String[0]),
                    res2.toArray(new String[0]));
        }
        scoring.updateScore("lang:common_list_test_scoring_name", 15);
    ...
    public void testabecedaMap() {
        {
            ...
            Map<String, Stream<String>> res1 = myabecedaMap(ls);
            Map<String, Stream<String>> res2 = Basics.abecedaMap(ls);

            for(String key : res2.keySet()) {
                System.out.println(key);
                System.out.println(res1.get(key));
                System.out.println(res2.get(key));
                if (res1 == null)
                    System.out.println("res1");
                if (res2 == null)
                    System.out.println("res2");
                assertArrayEquals("abecedaMap test 1",
                        res1.get(key).collect(Collectors.toList()).toArray(new String[0]),
                        res2.get(key).collect(Collectors.toList()).toArray(new String[0]));
            }
        }
        {
//            List<String> ls = Arrays.asList(spoluziaci);
            ...
            for(String key : res2.keySet())
                assertArrayEquals("abecedaMap test 2",
                        res1.get(key).collect(Collectors.toList()).toArray(new String[0]),
                        res2.get(key).collect(Collectors.toList()).toArray(new String[0]));
        }
        scoring.updateScore("lang:common_list_test_scoring_name", 20);
