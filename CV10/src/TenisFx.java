...
public class TenisFx extends Application implements Runnable {
    ...
    public void run() {
        ...
        while (!gameOver) {
            ...
            double  deviation = (new Random().nextDouble())-0.5;
            if (ny < 0) {
                // ny = -y;
                ballDirection = Math.PI - ballDirection;
                ballDirection += deviation;
            }
            if (ny > height) {
                // ny = 2 * height - ny;
                ballDirection = Math.PI - ballDirection;
            ...
            if (Math.abs(x1 - nx) < (bsize/2) && (ny >= y1) && (ny <= y1 + rsize)) {
                // nx = 20 - nx;
            ...
            if (Math.abs(x2 - nx) < (bsize/2) && (ny >= y2) && (ny <= y2 + rsize)) {
                // nx = 2 * width - 20 - nx;
                ballDirection = 2 * Math.PI - ballDirection;
                ballDirection += deviation;
            }
